...
RecordCommandsResult RainEmitterInst::renderFrame(uint32_t swapchain_image, VkRenderPassBeginInfo& render_pass_info) {
    ...
    vkCmdBindDescriptorSets(command_buffers[0], VK_PIPELINE_BIND_POINT_GRAPHICS, graphics_pipeline_.vk_pipeline_layout, COMPUTE_PARTICLE_BUFFER_SET_ID, 1, &vk_descriptor_sets_graphics_[offset], 0, nullptr);

    Pc pc = {compute_camera_.view_matrix, compute_camera_.proj_matrix};

    vkCmdPushConstants(command_buffers[0], graphics_pipeline_.vk_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(Pc), &pc);
